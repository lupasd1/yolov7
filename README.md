...
5. Run `pip3 install torch==1.13.1 torchvision==0.14.1 --index-url https://download.pytorch.org/whl/cu116` to install pyTorch with cuda support.
6. Run `pip install git+https://github.com/facebookresearch/detectron2` to install detectron2.
7. Cd into `yolov7\venv\lib\site-packages\torch\nn\modules` and open `upsampling.py`, find the forward function and remove `recompute_scale_factor=self.recompute_scale_factor` (don't ask why).
8. Run instance.py to test a singular video frame or open `yolov7_panoptic_final.rtd` in RTMaps for video.
9. In the Python Bridge component provide the path to the `yolov7_bridge.py` file and the path to your Python executable, which will probably be in `\venv\Scripts\python.exe`.
